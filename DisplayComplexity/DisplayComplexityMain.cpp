...
void DisplayComplexityMain::SetHolographicSpace(HolographicSpace^ holographicSpace)
{
	...
	for (unsigned int n = 0; n < N; ++n) {
		m_meshRenderers.push_back(std::make_unique<MeshRenderer>(m_deviceResources));
		m_meshRenderers.back()->SetPosition(float3(-0.3f + n * 0.025f * 0.5f, 0, -2));
	}
	...
	*/
	...
	for (auto cameraPose : prediction->CameraPoses)
	{
		...
		*/
		renderingParameters->SetFocusPoint(
			currentCoordinateSystem,
			m_meshRenderers.front()->GetPosition()
		);
#endif
	}
